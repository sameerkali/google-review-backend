...
r.get("/me", ah(async (req, res) => {
  const business = await Business.findById(req.businessId).populate("planId");
  if (!business) return res.status(404).json({ error: "not found" });
  res.json(business);
}));

r.get("/me/qr", ah(async (req, res) => {
  const hardware = await Hardware.find({ assignedBusinessId: req.businessId, status: "assigned" });
  res.json(hardware.map((h) => ({ serial: h.serial, type: h.type })));
}));

r.get("/me/reviews", ah(async (req, res) => {
  res.json(await ReviewSuggestion.find({ businessId: req.businessId }).sort({ createdAt: -1 }));
}));
...
r.delete("/me/reviews/:id", ah(async (req, res) => {
  const deleted = await ReviewSuggestion.findOneAndDelete({ _id: req.params.id, businessId: req.businessId });
...
r.get("/me/analytics", ah(async (req, res) => {
  ...
  const payload = {
    ...
    summary: { total, byType: { scan, google_click, review_copy }, conversionRate: scan ? Math.round((google_click / scan) * 100) : 0 },
  };
  ...
  if (features.userData) {
    payload.recentEvents = await AnalyticsEvent.find(q).sort({ createdAt: -1 }).limit(50).select("eventType device browser os createdAt");
  }
...
r.get("/me/suggestions", ah(async (req, res) => {
  ...
  const q = { businessId: req.businessId };
  const [scans, clicks, reviewCount, hardwareCount] = await Promise.all([
    AnalyticsEvent.countDocuments({ ...q, eventType: "scan" }),
    AnalyticsEvent.countDocuments({ ...q, eventType: "google_click" }),
